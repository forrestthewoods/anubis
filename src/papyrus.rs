...
pub fn resolve_value(
  ...
  match value {
    Value::Array(values) => {
      let new_values =
        values.into_iter().map(|v| resolve_value(v, value_root, vars)).collect::<anyhow::Result<Vec<_>>>()?;
    ...
    Value::Glob(glob_patterns) => {
      ...
      for pattern in glob_patterns {
        ...
        for entry in
          glob::glob(pattern_str).with_context(|| format!("Failed to parse glob pattern: {}", pattern_str))?
        {
          match entry {
