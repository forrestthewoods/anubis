...
fn build_cc_binary(binary: Arc<CcBinary>, job: Job) -> anyhow::Result<JobOutcome> {
    ...
    // Phase 1: Create jobs for dependencies only
    ...
    // Phase 2: After dependencies complete, compile sources
    // This ensures any generated source files exist before compilation
    let binary2 = binary.clone();
    let dep_job_ids2 = dep_job_ids.clone();
    let job_desc = job.desc.clone();
    let compile_phase = move |compile_job: Job| -> anyhow::Result<JobOutcome> {
        compile_binary_sources(binary2, dep_job_ids2, extra_args, lang, compile_job, job_desc)
    };

    // If no dependencies, go directly to compile phase
    if dep_job_ids.is_empty() {
        return compile_phase(job);
    }

    // Create continuation job to compile sources after deps complete
    let continuation_job = job.ctx.new_job(format!("{} (compile)", job.desc), Box::new(compile_phase));

    // Defer until all dependencies are built
...
/// Phase 2 of binary build: compile sources after dependencies complete.
/// This ensures any source files generated by dependencies exist before compilation.
fn compile_binary_sources(
    binary: Arc<CcBinary>,
    ...
    // Start with dependency job IDs (needed for link phase to collect libraries)
    let mut all_child_jobs: Vec<JobId> = dep_job_ids;

    // Now safe to compile sources - all dependencies (including code generators) are done
    ...
    let continuation_job = job.ctx.new_job(format!("{} (link)", original_desc), Box::new(link_job));

    // Defer until all compile jobs complete
    Ok(JobOutcome::Deferred(JobDeferral {
        blocked_by,
        continuation_job,
    }))
...
fn build_cc_static_library(static_library: Arc<CcStaticLibrary>, job: Job) -> anyhow::Result<JobOutcome> {
    ...
    // Phase 2: After dependencies complete, compile sources
    // This ensures any generated source files exist before compilation
    let static_library2 = static_library.clone();
    let dep_job_ids2 = dep_job_ids.clone();
    let job_desc = job.desc.clone();
    let compile_phase = move |compile_job: Job| -> anyhow::Result<JobOutcome> {
        compile_static_library_sources(static_library2, dep_job_ids2, extra_args, lang, compile_job, job_desc)
    };
...
/// Phase 2 of static library build: compile sources after dependencies complete.
/// This ensures any source files generated by dependencies exist before compilation.
fn compile_static_library_sources(
    ...
    // Start with dependency job IDs (needed for archive phase to collect libraries)
    let mut all_child_jobs: Vec<JobId> = dep_job_ids;
    ...
    // Phase 3: Archive all objects from child jobs into result
    ...
    let archive_job = move |archive_job: Job| -> anyhow::Result<JobOutcome> {
        archive_static_library(&all_child_jobs, &target, &name, archive_job.ctx.clone(), lang)
    };

    // Create continuation job to perform archive
    let continuation_job = job.ctx.new_job(format!("{} (create archive)", original_desc), Box::new(archive_job));
